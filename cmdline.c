                          ...
                          int need_save, Conf *conf)
{
    ...
    if (p[0] != '-') {
        ...
        if ((cmdline_tooltype & TOOLTYPE_HOST_ARG) &&
             ...
             * else).
             */
            if (!strncmp(p, "telnet:", 7) || !strncmp(p, "ssh:", 4)) {
				if (!strncmp(p, "telnet:", 7)) {
					...
					conf_set_int(conf, CONF_port, 23); /* we will rewrite it later if the port is specified explicitly */
				}else if (!strncmp(p, "ssh:", 4)){
					/*
					* If the argument starts with "ssh:", set the
					* protocol to SSH and process the string as a
					* SSH URL.
					*/

					/*
					* Skip the "ssh:" or "ssh://" prefix.
					*/
					p += 4;
					if (p[0] == '/' && p[1] == '/')
						p += 2;
					conf_set_int(conf, CONF_protocol, PROT_SSH);
					conf_set_int(conf, CONF_port, 22); /* we will rewrite it later if the port is specified explicitly */
				}
				...
				/*
				 * Originally if in protocol://host:port port is absent, it set to -1
				 * Reason of this is unclear.
				 * If protocol is defined, default port also defined and setted above,
				 * so this part commented.
				else {
    ...
    if (!strcmp(p, "-ssh")) {
		...
		do {
			if (cmdline_check_unavailable(TOOLTYPE_FILETRANSFER | TOOLTYPE_NONNETWORK, p))
				return ret; 
		} while (0);
