...
/*
 * Type and array for colorize black/white text via regex
 */
...
#define  COLOR_INTERFACE { 101, 167, 226 }
#define  COLOR_IP_ADDR { 219, 138, 67 }
#define  COLOR_VLAN { 226, 101, 219 }
#define  COLOR_MAC_ADDR { 128, 101, 226 }
...
struct colorize_pattern colorize_pattern_arr[] = {
	...
	XGigabitEthernet3/0/24
	*Jan 16 09:22:23.671: %LINK-3-UPDOWN: Interface GigabitEthernet0/0, changed state to up
	*/
	{ "(?<!\\w)(interface\\s)?(Fast|Gigabit|XGigabit)?Ethernet\\d+(/\\d+)?(/\\d+)?(?=\\W)", COLOR_INTERFACE },
	/*
	Tunnel0
	Tunnel0/1
	Tunnel0/0/381
	*/
	{ "(?<!\\w)(interface\\s)?Tunnel\\d+(/\\d+)?(/\\d+)?(?=\\W)", COLOR_INTERFACE },
	/*
	vlan 0
	voice-vlan 312 enable
	port hybrid pvid vlan 141
	port hybrid tagged vlan 2
	*/
	{ "(?<!\\w)(voice-)?vlan\\s\\d+(?=\\W)", COLOR_VLAN },
	/*
	2021-1-11 14:01:57+07:00 HOSTNAME %%01NTP/4/PEER_SELE(l)[22]:The peer selected by the system is 192.168.1.1.
	*/
	...
	/* IP V6 addresses*/
	{ "(?<!\\w)(([0-9a-fA-F]{1,4}:){7,7}[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,7}:|([0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,5}(:[0-9a-fA-F]{1,4}){1,2}|([0-9a-fA-F]{1,4}:){1,4}(:[0-9a-fA-F]{1,4}){1,3}|([0-9a-fA-F]{1,4}:){1,3}(:[0-9a-fA-F]{1,4}){1,4}|([0-9a-fA-F]{1,4}:){1,2}(:[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:((:[0-9a-fA-F]{1,4}){1,6})|:((:[0-9a-fA-F]{1,4}){1,7}|:)|[Ff][Ee]80:(:[0-9a-fA-F]{0,4}){0,4}%[0-9a-zA-Z]{1,}|::((ffff|FFFF)(:0{1,4}){0,1}:){0,1}((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])|([0-9a-fA-F]{1,4}:){1,4}:((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\\.){3,3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9]))(/\\d{1,3})?(?!(:|\\w))", COLOR_IP_ADDR },
	/*
	...
	*/
	{ "(?<!\\w)[0-9a-f]{4}[.-][0-9a-f]{4}[.-][0-9a-f]{4}(?=\\W)", COLOR_MAC_ADDR },
	/* Huawei:
	GigabitEthernet0/0 is up, line protocol is up
	*/
	{ "(?<=\\scurrent\\sstate\\s\\:\\s)UP(?=\\W)", COLOR_GREEN },
	/* Cisco:
	GigabitEthernet0/0 is up, line protocol is up
	*/
	{ "(?<=\\sline\\sprotocol\\sis\\s)up(?=\\W)", COLOR_GREEN },
	/* Huawei:
	GigabitEthernet3/0/20 current state : DOWN
	Line protocol current state : DOWN
	*/
	{ "(?<=\\scurrent\\sstate\\s\\:\\s)DOWN(?=\\W)", COLOR_RED },
	/*
	GigabitEthernet0/1 is down, line protocol is down
	GigabitEthernet0/1 is administratively down, line protocol is down
	*/
	{ "(?<=\\sline\\sprotocol\\sis\\s)down(?=\\W)", COLOR_RED },
	/*
	interface GigabitEthernet0/1
	 ...
	 .........
	*/
	{ "^\\s+shutdown(?=\\s*$)", COLOR_SHUTDOWN },
	/* this case abnormal, so color is strong red fo all line
	Cisco:
	FastEthernet1/0/15 is down, line protocol is down (err-disabled)
	*/
	{ "^.+\\sline\\sprotocol\\sis\\sdown\\s\\(err-disabled\\).*", COLOR_STRONG_RED },
	/* this case abnormal, so color is strong red fo all line
	Huawei:
	10GE1/0/1 current state : ERROR DOWN(reason) (ifindex: 53)
	*/
	{ "^.+\\scurrent\\sstate\\s\\:\\sERROR\\s\\DOWN\\(.+\\).*", COLOR_STRONG_RED } /* */,
	/* Huawei:
	<HOSTNAME>disp intrface
	                  ^
	...
	*/
	{ "^Command\\sauthorization\\sfailed\\s*$", COLOR_STRONG_RED },
};
...
static void term_copy_stuff_from_conf(Terminal *term)
{
    ...
    term->colorize = conf_get_bool(term->conf, CONF_colorize);
...
static void colorize_substr(termchar *termchars, const unsigned int from, const unsigned int n, color color) {
	for (int i = from; i < from + n; i++) {
		(termchars + i)->truecolour.fg.r = color.r;
		(termchars + i)->truecolour.fg.g = color.g;
		(termchars + i)->truecolour.fg.b = color.b;
		(termchars + i)->truecolour.fg.enabled = true;
	}
}
...
static void colorize(termline *ldata)
{
	char ch[1024];
	int n = ldata->cols;
	if (n > 1022) { n = 1022; }

	termchar *chptr = ldata->chars;
	int chpos = 0;
	while (chpos < n) {
		unsigned long chr = (chptr + chpos)->chr;
		char c = 0x20; /* space by default if not ASCII */
		if (((chr & CSET_MASK) == CSET_ASCII) || ((chr & CSET_MASK) == 0)) {
			/* ASCII non-unicode character */
			c = chr & 0xFF;
		}
		ch[chpos] = c;
		chpos++;
	...
	/*
	before colorization set to all charactrers in line default color
	*/
	for (int i = 0; i < n; i++) {
		(chptr+i)->truecolour.fg.enabled = false;
		(chptr+i)->truecolour.fg.r = 0;
		(chptr+i)->truecolour.fg.g = 0;
		(chptr+i)->truecolour.fg.b = 0;
	}

	ch[chpos] = 0;
	find_patterns(ldata, &ch[0]);
}
...
static void do_paint(Terminal *term)
{
    ...
    for (i = 0; i < term->rows; i++) {
		...
		if (term->colorize) colorize(ldata);
